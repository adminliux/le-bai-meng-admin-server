...
<div class="page-container">
    ...
    <div class="mt-20" >
        ...
        <script type="text/html" id="f1">
            <tr>
                <td style="text-align:center;vertical-align:middle;"><a href="${path}/page/live/course/update/live/byid?id={{data.id}}"><font color="#5a98de">修改</font></a></td>
                <td style="text-align:center;vertical-align:middle;">{{data.id}}</td>
                <td style="text-align:center;vertical-align:middle;">{{data.name}}</td>
                <td style="text-align:center;vertical-align:middle;"><u style="cursor:pointer" class="text-primary" onclick="member_show(' ','${path}/page/user/chat/room/user/show?id='+'{{data.userId}}','10001','360','400')"><font color="#4169e1">{{data.userId}}</font></u></td>
                <td style="text-align:center;vertical-align:middle;">{{if data.classifyId=="1"}}瑜伽{{else if data.classifyId=="2"}}团舞{{else if data.classifyId=="3"}}声乐{{else if data.classifyId=="4"}}模特{{else if data.classifyId=="5"}}其他{{/if}}</td>
                <td style="text-align:center;vertical-align:middle;">
                    {{if data.status=="TRAILER"}}
                    <span class="label badge-warning radius">未开播</span>
                    {{else if data.status=='DIRECT_SEEDING'}}
                    <span class="label badge-primary radius">直播中</span>
                    {{else if data.status=='REVIEW'}}
                    <span class="label badge-success radius">已结束</span>
                    {{/if}}
                </td>
                <td>{{if data.previewImg==NULL}}无{{else if data.previewImg!=NULL}}<a class="sit-preview" href="{{data.previewImg}}" onMouseOver="biggerr()"><img src="{{data.previewImg}}" data-preview-url="{{data.previewImg}}" style="cursor:pointer;width:60px;height:60px;"/></a>{{/if}}</td>
                <td style="text-align:center;vertical-align:middle;">{{if data.address==NULL}}暂无录播信息{{else if data.address!=NULL}}<u style="cursor:pointer" class="text-primary" onclick="member_show(' ','${path}/page/video/detail?address='+'{{data.address}}','10001','1060','800')"><font color="#4169e1">查看详情</font></u>{{/if}}</td>
                <td style="text-align:center;vertical-align:middle;">{{$timestampFormatYMDHM data.beginTime}}</td>
                <td style="text-align:center;vertical-align:middle;">{{$timestampFormatYMDHM data.overTime}}</td>
                <td style="text-align:center;vertical-align:middle;">{{data.shareSum}}</td>
                <td style="text-align:center;vertical-align:middle;">{{data.collectionSum}}</td>
                <td style="text-align:center;vertical-align:middle;">
                    {{if data.intro==NULL}}无{{else}}
                    <u style="cursor:pointer" class="text-primary" onclick="layer_show('简介','${path}/page/msg/detail?detail={{data.intro}}','800','600')"><font color="#4169e1">查看简介</font></u>
                    {{/if}}
                </td>
            </tr>
        </script>
    ...
    <script type="text/javascript">
        ...
        function findUser(pageNum){
            ...
            Project.ajax("/live/course/select/byid",p,null,true).ajaxOK(function(data) {
